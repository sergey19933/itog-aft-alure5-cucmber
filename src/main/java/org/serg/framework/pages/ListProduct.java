...
public class ListProduct extends BasePage {
    ...
    //span[contains(text(),'оставит')]//../..//div[contains(text(),'В корзину')]//..//../..//..//..//..//..//span[@style='color: rgb(249, 17, 85);']

    //span[contains(@style,'color:')]
    ...
    private WebElement choiceFilterField(String nameFilter) {
        ...
        Assertions.fail("Фильтр '" + nameFilter + "' не найдено");
        return null;
    }
...
//            Matcher matcher = pattern.matcher(activeFilter);
    ...
    //выбор фильтра чекбокса и работа с ним
...
//        }
        ...
        for (WebElement label : blockFilter.findElements(By.xpath(".//label"))) {
            if (label.getText().contains(nameChek)) {
                if (!label.isSelected() && onOff.equalsIgnoreCase("Включить")) {
                    ...
                    Assertions.fail("Неверные данные");
                }
                return;
            }
        }
        Assertions.fail("Чекбокс '" + nameChek + "' не найдено");
    }
    ...
    private void selectChekSwitcher(String nameFilter, WebElement blockFilter, String onOff) {
        ...
        Assertions.fail("Чекбокс '" + nameFilter + "' не найдено");
    }
//Добавление товара
    //------------------------------------------------------
    ...
    Assertions.assertTrue(checkCountAdd, "Не добавлено");
